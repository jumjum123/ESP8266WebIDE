...
// JUMJUM var acorn = require('./acorn_mod');
...
// jumjum var NODE_ENCODING_CESU8 = (Buffer('\ud800')[0] === 0xED);
    ...
    if (node.value instanceof RegExp) {
      ...
      return colony_node(node, '("' + (node.value
        ...
        .replace(/[^\u0020-\u007E]/g, function (s) {
          var seq;
          // jumjum if (NODE_ENCODING_CESU8) seq = Buffer(s);
          //else {
...
function colonize (script, opts)
{
  ...
  var nolinenumbers = (opts || {}).noLineNumbers !== false;
  var noexports = (opts || {}).noExports !== false;
  ...
  var mapped;
  if(noexports){
    mapped = [prelude,body].join(joiner)} 
  else {
    mapped = [
      prelude,
  ...
  if(nolinenumbers){ mapped = mapped.replace(/--\[\[\d+\]\]\s/g,"");}
  mapped = mapped.replace(/^\s*\n/gm,"");  
  return {
    source: mapped,
    sourcemap: sourcemap
  };
...
// Public API
// jumjum module.exports = colonize;
